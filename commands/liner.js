...
module.exports = {
  ...
  async execute(senderId, args, pageAccessToken, sendMessage) {
    ...
    try {
      const apiUrl = `https://joshweb.click/api/liner?q=${encodeURIComponent(query)}`;
      ...
      if (status) {
        ...
        console.error('Error: Unsuccessful response from Linerva Pro API.');
        sendMessage(senderId, { text: 'Sorry, there was an error processing your request.' }, pageAccessToken);
      }
    } catch (error) {
      console.error('Error calling Linerva Pro API:', error);
      sendMessage(senderId, { text: 'Sorry, there was an error processing your request.' }, pageAccessToken);
    }
  }
};
